...
var checkLables = function(labels, visNum, step_size) {
    res = [];

    if (labels.length % visNum == 0) {
        var gap = Math.floor(labels.length / visNum);
    } else {
        var gap = Math.floor(labels.length / visNum) + 1;
    }
    for (var i = 0; i < visNum*gap; i++) {
        if (i % gap == 0) {
            res.push(Math.floor(i * step_size));
        } else {
            res.push('');
        }
    }
...
var cnnvis = (function(exports) {
    ...
    Graph.prototype = {
        ...
        add: function(id, step_size, y) {
            visNum = 10;
            this.labels.push(id);
            this.chart.data.labels = checkLables(
                this.labels,
                visNum,
                step_size
            );
        ...
        drawSelf: function(canv) {
            ...
            var t = function(x, y, s) {
                ...
                var ty =
                    H -
                    ((y - s.miny) / (s.maxy - s.miny) * (H - pad * 2) + pad);
                return { tx: tx, ty: ty };
            };
