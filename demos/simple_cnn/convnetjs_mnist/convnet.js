      ...
      for(var d=0;d<this.out_depth;d++) {
        ...
        for(var ax=0; ax<this.out_sx; x+=this.stride,ax++) {
          ...
          for(var ay=0; ay<this.out_sy; y+=this.stride,ay++) {
            ...
            // convolve centered at this particular location
            var a = 0.0; //store sum
	    var nn =0.0; //store nieighbours number
            var winx=-1,winy=-1;
            for(var fx=0;fx<this.sx;fx++) {
              for(var fy=0;fy<this.sy;fy++) {
                var oy = y+fy;
...
// Implements polinomial nonlinearity from the output layer
  // x -> x
  // so the output remains the same
  var PolyLayer = function(opt) {
    var opt = opt || {};
  ...
  PolyLayer.prototype = {
    forward: function(V, is_training) {
      ...
      for(var i=0;i<N;i++) { 
	var Corr=0.5;
	V2w[i] = V2w[i]*V2w[i]*V2w[i]+V2w[i]*V2w[i];
	V2w[i] = V2w[i] * Corr;
      }	
      this.out_act = V2;
      return this.out_act;
	...
	var Corr=0.5;
	//V.dw[i] = V2.dw[i]*Corr;
        //V.dw[i] =  Math.cbrt(V2.dw[i])*Corr;
	//V.dw[i] = this.out_act.get_grad(ax,ay,d);
	//V.dw[i] = 1;
	//var partone=Math.cbrt(  3*Math.sqrt(3)*Math.sqrt(27*V2.dw[i]*V2.dw[i]-4*V2.dw[i])-27*V2.dw[i]+2)/Math.cbrt(2);
	//var parttwo=Math.cbrt(2)/ Math.cbrt(3*Math.sqrt(3) *Math.sqrt(27*V2.dw[i]*V2.dw[i]-4*V2.dw[i]) -27*V2.dw[i]+2 );
	//V.dw[i] =10000*1/3*(-partone-parttwo-1)
	V.dw[i] = Corr*(3*V2.w[i]*V2.w[i]+2*V2.w[i]) * V2.dw[i];
      }
    },
    getParamsAndGrads: function() {
      return [];
  ...
  // Implements No nonlinearity from the output laye
  // x -> x
  // so the output remains the same
  var NoLayer = function(opt) {
    var opt = opt || {};
  ...
  NoLayer.prototype = {
    forward: function(V, is_training) {
      ...
      var V2w = V2.w;
      for(var i=0;i<N;i++) { 
        V2w[i] = V2w[i]; // leave values unchanged
      }
      this.out_act = V2;
      return this.out_act;
    },
  ...
  global.PolyLayer = PolyLayer;
  global.NoLayer = NoLayer;
  ...
  Net.prototype = {
    ...
    makeLayers: function(defs) {
      ...
      var desugar = function() {
	    ...
	    else if (def.activation==='poly') { new_defs.push({type:'poly'}); }
	    else if (def.activation==='no') { new_defs.push({type:'no'}); }
	  ...
	  case 'avg': this.layers.push(new global.AvgLayer(def)); break;
          case 'relu': this.layers.push(new global.ReluLayer(def)); break;
	  case 'poly': this.layers.push(new global.PolyLayer(def)); break;
          case 'no': this.layers.push(new global.NoLayer(def)); break;
    ...
    fromJSON: function(json) {
      ...
      for(var i=0;i<json.layers.length;i++) {
        ...
        if(t==='poly') { L = new global.PolyLayer(); }
        if(t==='no') { L = new global.NoLayer(); }
        if(t==='sigmoid') { L = new global.SigmoidLayer(); }
        if(t==='tanh') { L = new global.TanhLayer(); }
        if(t==='dropout') { L = new global.DropoutLayer(); }
        if(t==='conv') { L = new global.ConvLayer(); }
        if(t==='pool') { L = new global.PoolLayer(); }
        if(t==='avg') { L = new global.AvgLayer(); }
